...
int write_score_to_file(int score) {
    ...
    // Open the file "score.txt" in write mode
    fr = f_open(&fil, "score.txt", FA_WRITE | FA_CREATE_ALWAYS);
    if (fr) {
        printf("Error opening score.txt for writing: %d\n", fr);
        return -1;  // Return an error code on failure
    }

    // Write the score to the file
    if (f_printf(&fil, "%d", score) < 0) {
        printf("Error writing to score.txt\n");
        f_close(&fil);
...
int read_first_line() {
    ...
    fr = f_open(&fil, "score.txt", FA_READ);
    if (fr) {
        printf("Error opening score.txt: %d\n", fr);
        return -1;  // Return a specific value on error, e.g., -1
    }

    // Read the first line and close the file
    if (f_gets(line, sizeof(line), &fil)) {
        f_close(&fil);
        return atoi(line);  // Convert the line to an int and return it
    } else {
        printf("Failed to read from score.txt\n");
        f_close(&fil);
        return -1;  // Return a specific value on read failure
    }
}


char* retrieve(char *argv[]) {
    FIL fil;        /* File object */
    char *line = malloc(100 * sizeof(char)); /* Dynamically allocate line buffer */
    FRESULT fr;     /* FatFs return code */

    if (line == NULL) { // Check if memory allocation was successful
    ...
    if (fr) {
        print_error(fr, argv[1]);
        free(line); // Free the allocated memory if there's an error
        return NULL;
    }

    /* Read the first line and store it */
    if (!f_gets(line, 100, &fil)) {
